<?php
	...
	//Só efetua o cadastro do endereço caso este esteja preenchido, para não ocupar espaço vazio!

	if(isset($cep)){
		//Verifica se o endereço já existe!

		$VerifyAddress = $PDO->prepare("SELECT * FROM endereco WHERE cep = ?");
		$VerifyAddress->execute(array($cep));

		$existe = $VerifyAddress->rowCount();

		if(!($existe > 0)){
			$insertAddress = $PDO->prepare("INSERT INTO endereco (cep, logradouro, bairro, cidade, uf) VALUES (?,?,?,?,?)");
			$insertAddress->bindParam(1, $cep);
			$insertAddress->bindParam(2, $logradouro);
			$insertAddress->bindParam(3, $bairro);
			$insertAddress->bindParam(4, $cidade);
			$insertAddress->bindParam(5, $uf);
